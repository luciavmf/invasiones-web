...
export class Menu extends GUIBox {
    ...
    addItem(index: number, stringId: number, flag: number): boolean {
        ...
        if (this.anchor & Surface.centerHorizontal) {
            this.posX = (Video.width >> 1) - (this.buttonWidth >> 1) + this.originalX
        }
        if (this.anchor & Surface.centerVertical) {
            this.posY = (Video.height >> 1) + this.originalY
                - (((this.buttonHeight + this.lineSpacing) * this.itemCount - this.lineSpacing) >> 1)
        }

        this.height = (this.buttonHeight + this.lineSpacing) * this.itemCount - this.lineSpacing
        this.width  = this.image?.width ?? this.buttonWidth
        return true
    }
    ...
    setPosition(x: number, y: number, anchor: number): void {
        ...
        if (anchor & Surface.centerHorizontal) {
            this.posX = (Video.width >> 1) - (this.buttonWidth >> 1) + x
        }
        if (anchor & Surface.centerVertical) {
            this.posY = (Video.height >> 1) + y
                - (((this.buttonHeight + this.lineSpacing) * this.itemCount - this.lineSpacing) >> 1)
        }
    }
    ...
    update(): number {
        ...
        for (let i = 0; i < this.itemCount; i++) {
            ...
            if (flags !== Menu.Constants.itemHidden) {
                ...
                if (mx > this.posX && mx < this.posX + this.buttonWidth
    ...
    draw(video: VideoType): void {
        if (this.image) {
            video.draw(this.image, this.posX, this.posY - 6, 0)
        } else {
            const padding     = 6
